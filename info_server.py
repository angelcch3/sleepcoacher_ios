...
class ServerHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        with open(self.store_path, 'r') as fh:
            ...
            # read_data = fh.read() #Jan 26, 2017 - got a "memory error" so the file is now too big, so we are no longer reading from it. 
            # # print read_data
	...
	if 'Carrier' in data_string and 'Gender' in data_string:
		if "User" in data_string:
			username = data_string.split("User=")[1]
		...
		# create path name for user_info.csv for each user
		try:
			filepath = pjoin(username,'user_info')
			...
			# write the data to a new csv for this user
