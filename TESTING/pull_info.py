# TODO - read from sleep.csv. to get the date, from and wakem times, hourslsept, and rating. but then do the math to calculte sol, aph, and awakenings.
import os
import csv
import datetime
import numpy as np
...
for filename in os.listdir(DATA_PATH):
	...
	with open(name, 'wb') as csvfile:	
		...
		for line in csv.reader(open('sleep.csv'), delimiter = ','):
			if 'timezone' in line:
				continue
			else:
				date = line[0].spli('T')
		...
		for fields in csv.reader(open(DATA_PATH + filename), delimiter=',', quotechar='"'):
			...
			if fields[0] == 'Id':
				...
				# hour = hour[:-1]
				date = fromTime[0][:-1]
				# print str(fromTime[0])+str(fromTime[1])+str(fromTime[2])
